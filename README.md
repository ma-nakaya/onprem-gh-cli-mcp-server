...
> [!IMPORTANT]
> 現在はPhase 2cです。読み取り操作に加え、型付きツールによるIssue、Pull Request、Draft Releaseの安全な書き込みを提供します。Repository削除、PRマージ、Release公開、Secret変更などの高影響操作は未実装です。
...
Issue・Pull Request・Draft Release書き込みには、対象Repositoryへの書き込み権限を持つGitHub CLI認証が必要です。
...
Issue・Pull Request・Draft Release書き込みの開始と完了をJSON Lines形式で記録します。
...
- 監査ログへIssue・Pull Request・Releaseの本文やコメント、Token、Secretを保存しない
...
- 未実装: Release公開、Workflow Dispatch、Project、Label、Milestone、削除、PRマージ、Secret、二段階承認
