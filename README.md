...
> [!IMPORTANT]
> 現在はPhase 2dです。読み取り操作に加え、型付きツールによるIssue、Pull Request、Draft Release、Workflow Dispatchを提供します。Repository削除、PRマージ、Release公開、Secret変更などの高影響な管理操作は未実装です。
...
Issue・Pull Request・Draft Release・Workflow Dispatchには、対象RepositoryとActionsへの必要権限を持つGitHub CLI認証が必要です。
...
Issue・Pull Request・Draft Release・Workflow Dispatchの開始と完了をJSON Lines形式で記録します。
...
- 監査ログへIssue・Pull Request・Releaseの本文やコメント、Workflow inputs、Token、Secretを保存しない
...
- 未実装: Release公開、Project、Label、Milestone、削除、PRマージ、Secret、二段階承認
